...
public class InventorySystem : MonoBehaviour {
    ...
    private GameObject CheckForEmptySlots() {
        ...
        foreach (var slot in inventorySlotObjects) {
            // If the child count is 1 (Only the outline) then return the current slot as available
    ...
    public bool IsFull() => inventorySlotObjects.Count(slot => slot.transform.childCount > 1) >= MAX_SLOTS;
    ...
    public void RemoveItemFromInventory(string itemIdentifier, bool alsoSpawnItemInWorld) {
        // Find the first slot holding this item and clear it (TODO: Modify later for player controlled item dropping)
        foreach (var (slot, id) in slotContents) {
            if (id != itemIdentifier) continue;
            
            // Destroy the item UI object (child index 1; index 0 is the outline)
