...
namespace vectorarts.scpcbr {
    ...
    public class RoomInstantiationModule {
        ...
        private async Task InstantiateDoor(RoomPlacementModule.DoorData doorData) {
            ...
            if (zone == null || zone.zoneDoorPrefab == null) return;
            ...
            // Ensure instantiation happens on the main thread
            await Task.Yield(); // Switch back to the main thread
            GameObject doorInstance = Object.Instantiate(zone.zoneDoorPrefab, position, rotation, roomParent);
            doorInstance.name = $"Door_{doorData.position.x}_{doorData.position.y}";
        }
    }
