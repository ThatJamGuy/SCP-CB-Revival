...
public class EVNT_Chamber173 : MonoBehaviour {
    ...
    public GameObject[] deletedTheseAfterBreach;
    ...
    private IEnumerator InChamberPart() {
        ...
        GlobalCameraShake.Instance.ShakeCamera(0.1f, 0f, 3);
        //DeleteThings();
        SceneManager.LoadSceneAsync("GamePostBreach", LoadSceneMode.Single);
        SceneManager.UnloadSceneAsync("GamePreBreach");
    }

    public void DeleteThings() {
        foreach(GameObject obj in deletedTheseAfterBreach) {
            Destroy(obj);
        }
    }
}